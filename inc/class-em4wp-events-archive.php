...
class EM4WP_Events_Archive extends EM4WP_Events_Core {
	...
	public function modify_archive_query( $query ) {
		...
		} elseif ( '' != get_query_var( $this->get_option( 'permalink-upcoming' ) ) && $query->is_main_query() && ! is_admin() && ( is_post_type_archive( 'event' ) || is_tax( 'event-category' ) ) ) {

			// Set the upcoming events query
			$meta_query = array(
				array(
					'key' => '_event_end',
					'value' => (int) current_time( 'timestamp' ),
